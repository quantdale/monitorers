...
- **PDH handles are opened once** in `CollectorState::new()` and never recreated — recreating resets rate-counter baselines (first reading is always 0%, by design). A single `PdhCollectQueryData` per tick snapshots GPU and disk counters atomically. Counter-array reads go through the reusable per-thread scratch buffer in `pdh.rs` (the GPU-engine array can span hundreds of KB with many processes and is read every ~250ms) — do not reintroduce allocate-per-call reads.
...
- **Supervised recovery**: each tick body runs inside `std::panic::catch_unwind`; a caught panic ends that SESSION with `LoopOutcome::Panicked` and `collector/supervisor.rs` replaces it — bounded automatic recovery (3 attempts per streak, staged backoff 500ms→8s, a healthy period ≥30s resets the streak) and a persistent `failed` state with manual retry via the `retry_collection` command (honored ONLY while failed: signals the retry flag and answers `Failed`; other answers are coalesced no-ops). Every session rebuilds fresh OS-facing state (PDH/sysinfo/NVML/WMI bootstrap/registry) on its own thread and primes rate baselines before waiting out its first deadline at the TOP of the loop, so post-recovery commits are real ~250ms deltas. Shutdown wins from every state. Tauri managed state is type-keyed: the stop/retry flags are distinct newtypes (`StopFlag`, `RetryRequest`) registered via `register_lifecycle_flags` — two raw `Arc<AtomicBool>` values would silently alias into one.

## Frontend architecture (`src/`)

- **`hooks/useMetrics.ts`** — single source of truth for metrics. `invoke("get_history", { windowSecs })` on mount/window-change for the initial snapshot, then `listen("metrics-update")` appends incrementally (`appendToHistory` / `mergeDiskHistory` / `mergeGpuHistory`) into ring buffers of `MAX_HISTORY = 3600`. On mount/reload the Tauri branch also bootstraps the current managed status via `get_collector_status` (after the status listener attaches; an applied-status sequence fence discards fetch responses older than an already-observed event), so an already-failed supervisor surfaces Retry without a new event or restart. `sliceWindow()` selects recorded timestamps by elapsed time. Schema mismatches fail closed with an actionable UI error; stale history requests are generation-guarded and replay live full-tick events. In the browser (no `window.__TAURI_INTERNALS__`) it uses the scriptable mock backend (lifecycle parity: healthy only after a generation's first snapshot; `stop()` invalidates crash/recovery timers via a run token). Perf-critical structure (do not flatten back into one memo): windowed channel slices are computed in a memo keyed on history/windowSeconds (≤1 Hz) while live scalars merge in a second cheap memo (~4 Hz); the live-event replay buffer is capped via `appendLiveEvent` (amortized O(1) push, bounded retention) rather than copying up to MAX_HISTORY snapshots per event.
