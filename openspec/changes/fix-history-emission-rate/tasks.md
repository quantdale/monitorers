...
- [x] 8.7 Manually verify in `npm run tauri dev`: let the app run several minutes, confirm the "1 hour" window's elapsed real time now tracks 1:1 (spot-check against a stopwatch or timestamp deltas), and confirm CPU/GPU card numbers still visibly update multiple times per second. **Fulfilled by the automated procedure in `add-autonomous-cadence-verification`** (cadence probe + checker PASS on a sensor-equipped Windows host — see `sys-monitor-tauri/docs/cadence-verification.md`); the manual stopwatch run is downgraded to optional corroboration, not the required gate.
