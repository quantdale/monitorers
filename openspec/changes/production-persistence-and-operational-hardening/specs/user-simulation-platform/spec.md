...
- **THEN** the post-relaunch rendered `[data-sb-id]` order exactly equals the pre-restart persisted order, the run used two distinct child processes on distinct CDP ports, in-memory history did not survive the restart, and metrics timestamps advance after the relaunch
