...
Defines the autonomous E2E verification contract: the running app is drivable by an automated webview harness on Windows (wait for metrics, read rendered text, count chart points, dispatch input, read back `settings.json`), and manual/exploratory scenarios are either converted to driven E2E assertions or explicitly retained in an exploratory register.
