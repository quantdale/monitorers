...
Ensures every `unsafe` block in the Rust backend carries a `// SAFETY:` comment explaining why it's sound, so the codebase's use of FFI (PDH, NVAPI, etc.) stays auditable as it grows, and duplicated unsafe code paths get consolidated rather than copy-pasted.
## Requirements
