...
function mockHistoryPayload(): HistoryPayload {
  ...
  return {
    ...
    disks: [
      { key: 'C:', values: Array.from({ length: n }, (_, i) => 10 + 30 * Math.sin(t(i) + 1)) },
      { key: 'D:', values: Array.from({ length: n }, (_, i) => 5 + 15 * Math.sin(t(i) + 2)) },
    ],
...
function mockMetricsSnapshot(): MetricsSnapshot {
  ...
  return {
    ...
    mem_total_gb: 16,
    disks: [
      { key: 'C:', active: 10 + 30 * Math.sin(t * 0.2 + 1) },
      { key: 'D:', active: 5 + 15 * Math.sin(t * 0.2 + 2) },
    ],
